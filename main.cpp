...
private:
    void initCrosswords() {
        // Уровень 1
        if (currentLevel == 1) {
            crossword.addWord("программирование", "Процесс создания компьютерных программ", 0, 0, true, 1);
            crossword.addWord("алгоритм", "Последовательность действий для решения задачи", 0, 2, false, 2);
            crossword.addWord("синтаксис", "Правила написания кода", 3, 4, true, 3);
            crossword.addWord("компилятор", "Программа, переводящая код в машинный язык", 5, 0, false, 4);
        }
